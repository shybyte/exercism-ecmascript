...
describe('TwoBucket', () => {
  ...
  describe('works for input of 3, 5, 5', () => {
    const buckOne = 3;
    const buckTwo = 5;
    const goal = 5;

    it('starting with bucket two', () => {
      const starterBuck = 'two'; //indicates which bucket to fill first
      const twoBucket = new TwoBucket(buckOne, buckTwo, goal, starterBuck);
      expect(twoBucket.moves()).toEqual(1); //includes the first fill
      expect(twoBucket.goalBucket).toEqual('two'); //which bucket should end up with the desired # of liters
      expect(twoBucket.otherBucket).toEqual(0); //leftover value in the "other" bucket once the goal has been reached
    });
  ...
  describe('works for input of 3, 5, 3', () => {
    const buckOne = 3;
    const buckTwo = 5;
    const goal = 3;
