...
describe('simple-linked-list', () => {
  ...
  describe('List', () => {
    ...
    test('allows you to reverse a longer List', () => {
      const ll = List.fromArray([1, 2, 3, 4]);
      ll.reverse();
      expect(ll.toArray()).toEqual([4, 3, 2, 1]);
    });

    test('allows you to reverse a empty List', () => {
      const ll = List.fromArray([]);
      ll.reverse();
      expect(ll.toArray()).toEqual([]);
    });

    test('allows you to reverse a list with 1 element', () => {
      const ll = List.fromArray([1]);
      ll.reverse();
      expect(ll.toArray()).toEqual([1]);
    });

    test('allows you to reverse a list with 2 element', () => {
      const ll = List.fromArray([1]);
      ll.reverse();
      expect(ll.toArray()).toEqual([1]);
    });
